...
import { CleaningService } from '../../domains/cleaning/cleaning.service';
import { InventoryService } from '../../domains/inventory/inventory.service';
import { KeysReadService } from '../../domains/keys/keys.read.service';
import type {
  CleaningBatchRequestDto,
  CleaningBatchResponseDto,
  CleaningBatchResultDto,
  CleaningTargetStatus,
} from '../../domains/cleaning/dto/cleaning.dto';

@Injectable()
export class CleaningOrchestratorService {
  constructor(
  ...
  async createBatch(
    ...
    const batch = await this.cleaningService.createBatch(this.databaseService.client, {
  ...
  private async transitionItem(
    ...
    try {
      const outcome = await this.databaseService.transaction(async (trx) => {
        const { updated, fromStatus } = await this.inventoryService.transitionStatus(
          trx,
          itemId,
          toStatus,
          {
        ...
        await this.cleaningService.createBatchItem(trx, {
          batch_id: batchId,
