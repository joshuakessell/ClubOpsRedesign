...
import { requestCheckout, type CheckoutEventDto } from '../../lib/api';

export default function CheckoutRequestPage() {
  const [auth, setAuth] = useState<AuthState>({ device: null, staffToken: null });
  const [visitId, setVisitId] = useState('');
  const [result, setResult] = useState<CheckoutEventDto | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const response = await requestCheckout(auth.device, auth.staffToken, visitId);
      setResult(response);
    } catch (err) {
      setResult(null);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-white to-cyan-50">
      <header className="mx-auto max-w-3xl px-6 pt-12">
        ...
        <h1 className="mt-3 text-3xl font-semibold text-foreground">Checkout Request</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Request a kiosk checkout for a visit.
        </p>
        <div className="mt-4">
          <Nav />
        </div>
          ...
          <section className="rounded-xl border bg-card/90 p-4">
            <div className="grid gap-3">
              ...
              <Button onClick={handleSubmit} disabled={!visitId || status === 'loading'}>
                {status === 'loading' ? 'Submitting…' : 'Request Checkout'}
              </Button>
            </div>
